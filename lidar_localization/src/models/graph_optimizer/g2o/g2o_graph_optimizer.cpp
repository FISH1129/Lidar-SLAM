...
G2oGraphOptimizer::G2oGraphOptimizer(const std::string &solver_type)
{
  ...
  g2o::OptimizationAlgorithm *solver = solver_factory->construct(solver_type, solver_property);//3、创建总求解器Solver，LM
  graph_ptr_->setAlgorithm(solver);  //4、稀疏优化器，设置求解器
...
bool G2oGraphOptimizer::Optimize()
{
  ...
  graph_ptr_->setVerbose(false); //4、稀疏优化器，不打开调试输出
