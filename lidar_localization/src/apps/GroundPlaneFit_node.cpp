...
#define SLRPointXYZIRL lidar_localization::PointXYZIRL
#define VPoint velodyne_pointcloud::PointXYZIR
#define RUN pcl::PointCloud<SLRPointXYZIRL>
// Register custom point struct according to PCL
POINT_CLOUD_REGISTER_POINT_STRUCT(lidar_localization::PointXYZIRL,
                                  (float, x, x)
                                  (float, y, y)
                                  (float, z, z)
...
/*
    ...
    @subscirbe:/synced_cloud
    @publish:  /points_noground_synced, /points_ground_synced
*/
...
GroundPlaneFit::GroundPlaneFit():node_handle_("~"){
    // Init ROS related
    std::cout << "-----------------地面去除-------------------" << std::endl;
    ROS_INFO("Inititalizing Ground Plane Fitter...");
    node_handle_.param<std::string>("point_topic", point_topic_, "/synced_cloud");
    ...
    // Publish Init
    std::string noground_synced_topic, ground_synced_topic;
    node_handle_.param<std::string>("no_ground_point_topic", noground_synced_topic, "/points_noground_synced");
    ROS_INFO("No Ground Output Point Cloud: %s", noground_synced_topic.c_str());
    node_handle_.param<std::string>("ground_point_topic",    ground_synced_topic,   "/points_ground_synced");
    ROS_INFO("Only Ground Output Point Cloud: %s", ground_synced_topic.c_str());

    
    groundless_points_pub_ = node_handle_.advertise<sensor_msgs::PointCloud2>(noground_synced_topic, 2);
    ground_points_pub_     = node_handle_.advertise<sensor_msgs::PointCloud2>(ground_synced_topic, 2);
...
void GroundPlaneFit::extract_initial_seeds_(const pcl::PointCloud<VPoint>& p_sorted){
    ...
    for(unsigned int i=0;i<p_sorted.points.size() && cnt<num_lpr_;i++){
    ...
    for(unsigned int i=0;i<p_sorted.points.size();i++){
...
void GroundPlaneFit::velodyne_callback_(const sensor_msgs::PointCloud2ConstPtr& in_cloud_msg){
    ...
    sort(laserCloudIn.points.begin(),laserCloudIn.end(),point_cmp); //开始的是z小的点
    ...
    for(unsigned int i=0; i<laserCloudIn.points.size(); i++){   // 统计小于-1.5*sensor_height_的点数目
    ...
    for(int i=0;i<num_iter_;i++){
        ...
        for(int r=0; r<result.rows(); r++){
            if(result[r]<th_dist_d_){
                g_all_pc->points[r].label = 1u;// means ground  到拟合的平面的距离小于th_dist_的点 作为最后的地面点
...
int main(int argc, char **argv)
{
    ...
    /*
    ros::init(argc, argv, "GroundPlaneFit_node");
