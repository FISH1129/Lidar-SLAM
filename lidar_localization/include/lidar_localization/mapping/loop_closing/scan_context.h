...
public:
  // hyper parameters ()
  const double LIDAR_HEIGHT = 2.0; // lidar height : add this for simply directly using lidar scan in the lidar local
                                   // coord (not robot base coord) / if you use robot-coord-transformed lidar scans,
                                   // just set this as 0.
  ...
  const double SC_DIST_THRES = 0.5; // 0.4-0.6 is good choice for using with robust kernel (e.g., Cauchy, DCS) + icp
                                    // fitness threshold / if not, recommend 0.1-0.15

  // config
  const int TREE_MAKING_PERIOD_ =
      10; // i.e., remaking tree frequency, to avoid non-mandatory every remaking, to save time cost / in the LeGO-LOAM
