...
public:
	...
	/* Set input points */
	void setInput(CloudData::CLOUD_PTR input);
	...
	void radiusSearch(CloudData::POINT query_point, float radius, std::vector<int> &voxel_ids, int max_nn = INT_MAX);
	...
	double nearestNeighborDistance(CloudData::POINT query_point, float max_range);

	Eigen::Vector3d getCentroid(int voxel_id) const;
	double getSaticValue(int voxel_id) const;
	Eigen::Matrix3d getCovariance(int voxel_id) const;
	Eigen::Matrix3d getInverseCovariance(int voxel_id) const;

	void update(const CloudData::CLOUD_PTR new_cloud);
...
private:
	...
	void findBoundaries(const CloudData::CLOUD_PTR input_cloud,
							float &max_x, float &max_y, float &max_z,
							float &min_x, float &min_y, float &min_z);

	int voxelId(CloudData::POINT p);
	...
	void updateVoxelContent(const CloudData::CLOUD_PTR new_cloud);

	int nearestVoxel(CloudData::POINT query_point, Eigen::Matrix<float, 6, 1> boundaries, float max_range);
	...
	boost::shared_ptr<std::vector<double> > staticvalue_;
	...
	boost::shared_ptr<std::vector<double> > tmp_staticvalue_;
