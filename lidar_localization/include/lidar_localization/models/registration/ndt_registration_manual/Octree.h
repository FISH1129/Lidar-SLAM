...
public:
	...
	void setInput(std::vector<Eigen::Vector3i> occupied_voxels, const CloudData::CLOUD_PTR point_cloud);

	void update(std::vector<Eigen::Vector3i> new_voxels, const CloudData::CLOUD_PTR new_cloud);

	Eigen::Matrix<float, 6, 1> nearestOctreeNode(CloudData::POINT q);
...
private:
	...
	void updateOctreeContent(std::vector<Eigen::Vector3i> new_voxels, const CloudData::CLOUD_PTR new_cloud);

	double dist(OctreeNode node, CloudData::POINT q);

	/* Three functions to search for the nearest neighbor of a point */

	void initRange(CloudData::POINT q, double &min_range, int &current_nn_voxel);

	void goUp(Eigen::Matrix<int, 4, 1 > tree_node, CloudData::POINT q, double &min_range, int &current_nn_voxel);

	void goDown(Eigen::Matrix<int, 4, 1> tree_node, CloudData::POINT q, double &min_range, int &current_nn_voxel);
